...
export async function uploadPdfQuestionsAction(formData: FormData) {
  const title = formData.get('title') as string
  const description = formData.get('description') as string
  const subject_id = formData.get('subject_id') as string
  const questionsJson = formData.get('questions') as string

  if (!questionsJson) return { error: 'No questions provided' }
  ...
  try {
    ...
    // 1. Create Question Bank Container
    const bank = await prisma.question_banks.create({
      data: {
        title: title || `Smart Upload ${new Date().toLocaleDateString()}`, 
        description: description || 'Extracted via AI PDF OCR', 
        subject_id 
      },
      select: { id: true }
    })
    ...
    for (const [index, q] of questions.entries()) {
      try {
        ...
        const cleanCorrect = correctVal.toString().trim()

        const optionsData = rawOptions
          .filter(o => o.text && o.text.trim() !== '')
          .map(opt => {
            const isCorrect = (opt.text.trim() === cleanCorrect)
            return {
              question_id: question.id,
              text: opt.text,
              is_correct: isCorrect
        ...
        // Ensure at least one option is marked correct if possible (fallback logic if exact match fails)
        const hasCorrect = optionsData.some(o => o.is_correct)
        if (!hasCorrect && optionsData.length > 0) {
            // If the AI missed exact matching, we might need a fuzzier check, or just default to none (which is bad)
            // But usually exact match works if we asked AI to return exact string.
        }
