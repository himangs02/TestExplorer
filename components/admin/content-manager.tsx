...
const iconList = Object.keys(LucideIcons).filter(
  (key) => isNaN(Number(key)) && key !== "createLucideIcon" && key !== "default"
);
...
export default function ContentManager({ streams }: { streams: Stream[] }) {
  ...
  const toggleStream = (id: string) =>
    setOpenStreams((p) => ({ ...p, [id]: !p[id] }));
  const toggleExam = (id: string) =>
    setOpenExams((p) => ({ ...p, [id]: !p[id] }));
        ...
        <div className="bg-white rounded-3xl w-full max-w-5xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col md:flex-row">
          <div className="p-8 flex-1 overflow-y-auto">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setStreamForm((p) => ({ ...p, title: e.target.value }))
                    }
                    ...
                    <div className="grid grid-cols-6 gap-2 max-h-40 overflow-y-auto">
                      {filteredIcons.map((icon) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            setStreamForm((p) => ({ ...p, icon }));
                            setIsPickerOpen(false);
                          }}
                          className={`p-2 rounded-lg flex items-center justify-center hover:bg-white hover:shadow-sm transition-all ${
                            streamForm.icon === icon
                              ? "bg-black text-white"
                              : "text-gray-500"
                          }`}
                        >
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input
                    ...
                    onChange={(e) =>
                      setStreamForm((p) => ({ ...p, color: e.target.value }))
                    }
                    className="h-10 w-16 p-1 rounded-lg border border-gray-200 cursor-pointer"
                  />
                  <span className="font-mono text-sm bg-gray-100 px-3 py-2 rounded-lg text-gray-600">
                    {streamForm.color}
                  </span>
                </div>
              </div>
              <button
                type="submit"
                className="w-full bg-black text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition-all shadow-lg"
              >
        ...
        <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
          ...
          >
            ...
            {modal?.parentId && (
              <input
                type="hidden"
                name={isExam ? "category_id" : "course_id"}
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-4">
                  <input
                    type="checkbox"
                    name="is_published"
                    defaultChecked={modal?.data?.is_published}
                    id="pub"
                    className="w-4 h-4"
                  />
                  <label
            ...
            <button
              type="submit"
              className="w-full bg-black text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition-all mt-2"
            >
              Save Changes
            </button>
          </form>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      ...
      {(modal?.type === "exam" || modal?.type === "subject") &&
        renderSimpleModal()}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">
            Manage Streams, Exams, and Subjects in one place.
          </p>
        </div>
        <button
      ...
      <div className="space-y-4">
        {streams.map((stream) => {
          ...
          return (
            <div
              key={stream.id}
              className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm transition-all"
            >
              <div className="flex items-center justify-between p-4 bg-gray-50/50 border-b border-gray-100 group">
                ...
                >
                  ...
                  >
                    <DynamicIcon
                      name={stream.icon_key || "Circle"}
                      className="w-5 h-5 text-gray-900 opacity-70"
                    />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => openStreamModal("edit", stream)}
                  ...
                  <button
                    ...
                    className="ml-2 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-700 flex items-center gap-1 shadow-md hover:shadow-lg transition-all"
                  >
                    <Plus className="w-3 h-3" /> Exam
                  </button>
                </div>
              </div>
                ...
                <div className="p-4 bg-white space-y-3">
                  {stream.courses.length === 0 && (
                    <p className="text-sm text-gray-400 italic text-center py-4">
                      No exams yet. Click "+ Exam" to add one.
                    </p>
                  )}
                  {stream.courses.map((exam) => {
                    ...
                    return (
                      <div
                        key={exam.id}
                        className="border border-gray-100 rounded-2xl overflow-hidden transition-all hover:border-gray-300"
                      >
                        <div
                        ...
                        >
                          ...
                          >
                            ...
                            <button
                              ...
                              className="ml-2 bg-orange-100 text-orange-700 px-3 py-1 rounded-lg text-xs font-bold hover:bg-orange-200 flex items-center gap-1"
                            >
                        ...
                        {isExamOpen && (
                          <div className="bg-gray-50 p-3 space-y-2 border-t border-gray-100 animate-in slide-in-from-top-2 duration-200">
                            {subjectCount === 0 && (
                              <p className="text-xs text-gray-400 italic pl-10 py-2">
                                No subjects yet.
                              </p>
                            )}
